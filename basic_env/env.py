...
from base_env import PAMbase, PAMConfig
...
class PAMenv(gym.Env):
    def __init__(
        ...
        # hysr_one_ball_config = PAMConfig.from_json(hysr_one_ball_config_file)


        hysr_one_ball_config = HysrOneBallConfig.from_json(hysr_one_ball_config_file)
        reward_config_file = "config/reward_default.json"
        ...
        # self._hysr = PAMbase(hysr_one_ball_config) # reward_function)
    ...
    def step(self, action):
        ...
        observation, reward, episode_over = self._hysr.step(swing_pressures)
